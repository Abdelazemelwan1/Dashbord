...
export default function TableSection(): JSX.Element {
    ...
    const getStatusColor = (status: string): string  => {
        switch (status) {
            ...
            case "pending":
                return "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/3 dark:text-yellow-400";
            case "cancelled":
                return "bg-red-100 text-red-700 dark:bg-red-900/3 dark:text-red-400";
            default:
                return "bg-slate-100 text-slate-700 dark:bg-slate-900/3 dark:text-slate-400";
        }
    }
                    ...
                    <div className='p-6 border-b border-slate-200/50 dark:border-slate-700/50'>
                        <div className='flex items-center justify-between'>
                            <div>
                                ...
                                <p className='text-sm font-bold text-slate-500 dark:text-slate-400'>Latest customer Orders</p>
                            </div>
                            ...
                            <tbody>
                                {recentOrders.map((order , index) =>
                                    <tr key={index} className='border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors '>
                                        ...
                                        <td className='p-4'>
                                            <span className='text-sm text-slate-800 dark:text-white'>{order.products}</span>
                                        </td>
                                        <td className='p-4'>
                                            <span className='text-sm text-slate-800 dark:text-white'>{order.amount}</span>
                                        </td>
                                        <td className='p-4'>
                                            <span className={`text-slate-400 dark:text-white font-medium text-xs px-3 py-1 rounded-full ${getStatusColor(order.status)}`}>{order.data}</span>
                                        </td>
                                        <td className='p-4'>
                ...
                <div className='bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden'>
                    <div className='p-6 border-b border-slate-200/50 dark:border-slate-700/50'>
                        <div className='flex items-center justify-between'>
                            <div className='text-lg font-bold text-slate-800 dark:text-white'>
                                <h3 className='text-lg font-bold text-slate-800 dark:text-white'>Top Prdoucts</h3>
                            </div>
                            <p className='text-sm text-slate-500 dark:text-slate-400'>best Performing producats</p>
                        </div>
                        <button className='text-blue-600 hover:text-blue-700 text-sm font-medium'>view All</button>
                    </div>
                    {/*Dynamic Data  */}
                    <div className='p-6 space-y-4'>
                        {topProducts.map((product, index)=>
                            <div key={index} className='flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/5 transition-colors'>
                                <div className='flex-1'>
                                    <h4 className='text-sm font-semibold text-slate-800 dark:text-white'>{product.name}</h4>
                                    <p className='text-xs font-semibold text-slate-500 dark:text-slate-400'>{product.sales}</p>
                                </div>
